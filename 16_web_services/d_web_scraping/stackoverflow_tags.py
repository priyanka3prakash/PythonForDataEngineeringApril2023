...
def tag_crawler(question_url):
    ...
    for div in tag_divs:
        for tag_link in div.find_all("a"):
            ...
            if tag is not None:
                tag_count_dict[tag] = (
                    tag_count_dict[tag] + 1 if tag in tag_count_dict else 1
                )
...
def print_welcome_msg():
    ...
    print("\n\n")
    print(
        "This script will crwal stackoverflow pages to fetch the count of occurances of tags."
    )
    print(
        "And then print the top 10 tags hence predicting the most popular technology on stackoverflow."
    )
    print("Next we will ask you the number of pages to crawl.")
    print(
        "Remember, more the pages, better is the accuracy but at the same time script will run longer."
    )
...
def start():
    ...
    sorted_tag_count_dict = sorted(
        tag_count_dict.items(), key=operator.itemgetter(1), reverse=True
    )[:10]
    print("")
